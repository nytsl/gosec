...
func NewSubdomainsService() *SubdomainsService {
	return &SubdomainsService{}
}

func (s *SubdomainsService) Query(domains string) error {
	subfinderOpts := &runner.Options{
		Threads:            10, // Thread controls the number of threads to use for active enumerations
		Timeout:            30, // Timeout is the seconds to wait for sources to respond
		MaxEnumerationTime: 10, // MaxEnumerationTime is the maximum amount of time in mins to wait for enumeration
		Proxy:              config.GetGlobalProxy(),
	...
	if sourceMap, err = subfinder.EnumerateSingleDomainWithCtx(context.Background(), domains, []io.Writer{output}); err != nil {
